...
def matchup(request, auth):
    ...
    for user_id in user_dicts:
        ...
        filtered_group = []
        filtered_covid = [u for u in user_dicts if user_dicts[u]["covid"] == user_dicts[user_id]["covid"] and u != user_id]
        filtered_meet_type = [u for u in user_dicts if user_dicts[u]["meetType"] == user_dicts[user_id]["meetType"] and u != user_id]

        potential_matches[user_id] = {
            "potentials": list(set(filtered_group + filtered_covid + filtered_meet_type)),
            "number": len(list(set(filtered_group + filtered_covid + filtered_meet_type)))
        }
    ...
    rating = Rating(meeting_id=random_id, rating_dict={user: -1 for user in zero_matches})
